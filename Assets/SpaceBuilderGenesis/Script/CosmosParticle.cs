...
public class CosmosParticle : MonoBehaviour {
	...
	void Update(){
		...
		if (isReady){
			...
			for (int i = 0; i < particles.Length; i++) {
				...
				particles[i].startColor = new Color( particles[i].startColor.r/255f,particles[i].startColor.g/255f,particles[i].startColor.b/255f, alpha);
	...
	void On_CosmosReady (){
		...
		var em = cacheCosmosParticle.emission;
						em.enabled = false;
						em.rate = new ParticleSystem.MinMaxCurve(0,0 ) ;
		...
		for (int i=0; i < maxParticle; i ++) {
			...
			//cacheCosmosParticle.Emit( cacheTransform.position + (UnityEngine.Random.insideUnitSphere * 200), drift, Random.Range(minSize, maxSize ) , Mathf.Infinity, color.Evaluate( Random.Range(0f,1f)));
								...
								param.position = cacheTransform.position + (UnityEngine.Random.insideUnitSphere * 200);
								param.startSize = Random.Range(minSize, maxSize );
								param.startColor = color.Evaluate( Random.Range(0f,1f));
								param.velocity= drift;
								param.startLifetime = Mathf.Infinity;
								cacheCosmosParticle.Emit( param,1);
		}
